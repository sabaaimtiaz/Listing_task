...
import { fetchEditCandidate } from '../Redux/Slices/editSlice';
import { Spin } from 'antd';

const EditForm = ({ handleOk , id }) => {
 
  const dispatch = useDispatch();
  useEffect (()=>{              
       dispatch(fetchEditCandidate(id));
       console.log("data from edit",id);
  } ,[id]);
   const data = useSelector( state=> state)  
  useEffect(() => {    
    const dataSource = data?.EditUser?.editData?.records?.prospective_candidate
     console.log("data will show",data)
    console.log("datasource",dataSource)
    form.setFieldsValue({
        cell: dataSource?.cell,
        first_name: dataSource?.first_name,
        middle_name: dataSource?.middle_name,
        last_name: dataSource?.last_name,
        email:  dataSource?.email,
        gender: dataSource?.gender,
        recruiter: dataSource?.recruiter,
        shift: dataSource?.shift,
        mode_of_transportation: dataSource?.mode_of_transportation,
        city: dataSource?.city?.name || '',
        position_seeking:  dataSource?.position_seeking,
